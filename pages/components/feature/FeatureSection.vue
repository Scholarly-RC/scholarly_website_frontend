...
const { data, status, error } = useFetch(`${config.public.apiBaseUrl}/items/feature_section/`, { method: "get" })
</script>

<template>
    <section class="bg-white dark:bg-gray-900">
        <div class="py-8 px-4 mx-auto max-w-screen-lg sm:py-16 lg:px-6">
            <div v-if="status === 'pending'" class="max-w-screen-lg mb-8 lg:mb-16 animate-pulse">
            ...
            <div v-else>
                <div v-show="data?.data?.status === 'published'">
                    <h2 class="mb-4 text-4xl tracking-tight font-extrabold text-gray-900 dark:text-white">{{
                        data.data.title }}</h2>
                    <p class="mb-5 text-gray-500 sm:text-xl dark:text-gray-400">{{ data.data.description }}</p>
