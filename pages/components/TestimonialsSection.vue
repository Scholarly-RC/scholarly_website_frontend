...
<template>
    <ClientOnly>
        <section class="bg-white dark:bg-gray-900 px-4 py-8">
            <swiper-container v-if="status === 'success' && data?.data" ref="swaiperContainerRef">
                <swiper-slide v-for="testimonial in data.data" :key="testimonial.id" class="mb-10 px-4">
                    <figure class="w-full max-w-lg mx-auto text-center">
                        ...
                        <figcaption
                            ...
                            <img class="w-10 h-10 rounded-full"
                                :src="`${config.public.apiBaseUrl}/assets/${testimonial.image}/`" alt="Profile Picture">
                            <div
