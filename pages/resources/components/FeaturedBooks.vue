...
const { data, status, error } = useFetch(`${config.public.apiBaseUrl}/items/resources_items/`, { method: "get" })
</script>

<template>
    <section class="bg-white dark:bg-gray-900">
        <div v-if="status === 'pending'" class="py-8 px-4 mx-auto max-w-screen-lg lg:pb-10 lg:pt-5 lg:px-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
                <div v-for="index in 3" :key="index" class="w-full flex flex-col items-center animate-pulse">
                    <div class="w-11 h-6 bg-gray-200 dark:bg-gray-700 rounded-full mb-1"></div>
                    <div
