...
const { data, status, error } = useFetch(`${config.public.apiBaseUrl}/items/about_hero_section/`, { method: "get" })
</script>
<template>
        ...
        <div v-if="status === 'success' && data?.data"
            ...
            <div class="grid grid-cols-2 gap-4 mt-8">
                ...
                <img class="mt-4 w-full lg:mt-10 rounded-lg"
                    :src="`${config.public.apiBaseUrl}/assets/${data?.data?.image_2}/`" alt="About Page Image 2">
            </div>
        </div>
        <div v-else
