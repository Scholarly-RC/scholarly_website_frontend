...
const { data, status, error } = useFetch(`${config.public.apiBaseUrl}/items/faq_section/`, { method: "get" })
</script>
            ...
            <div v-else>
                <div v-show="data.data.status === 'published'">
                    <h2 class="mb-4 text-2xl tracking-tight font-extrabold text-gray-900 dark:text-white">{{
                        data.data.title }}</h2>
                    <FaqItem v-for="(faq_item, index) in data.data.faqs" :key="index" :faq_item="faq_item" />
                </div>
            </div>
        </div>
    </section>
