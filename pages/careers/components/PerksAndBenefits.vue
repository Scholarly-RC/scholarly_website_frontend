...
const { data, status, error } = useFetch(`${config.public.apiBaseUrl}/items/careers_perks_and_benefits/`, { method: "get" })
</script>
            ...
            <div v-show="data?.data?.status === 'published'">
                ...
                <div class="grid gap-4 sm:gap-8 grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
                    <div v-for="(item, index) in data.data.items" :key="index"
                        class="p-6 bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
                        <span class="material-symbols-outlined text-gray-700 dark:text-gray-300 text-4xl mb-4">{{
                            item.icons }}</span>
