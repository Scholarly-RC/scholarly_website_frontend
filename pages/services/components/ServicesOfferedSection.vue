...
const { data: services_offered_items_data, status: services_offered_items_status, error: services_offered_items_error } = useFetch(`${config.public.apiBaseUrl}/items/services_offered_items/`, { method: "get" })

</script>
<template>
    <section class="bg-white dark:bg-gray-900">
        <div class="py-8 px-4 mx-auto max-w-screen-lg sm:py-16 lg:px-6">
            <div v-if="services_offered_header_status === 'success' && services_offered_header_data?.data"
            ...
            <div v-else class="max-w-screen-md mb-8 lg:mb-16 animate-pulse">
            ...
            <div v-if="services_offered_items_status === 'success' && services_offered_items_data?.data"
                class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10">
                <ServicesOfferedCard v-for="services in services_offered_items_data.data" :key="services?.id"
                    :services="services" />
            </div>
            <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10 animate-pulse">
