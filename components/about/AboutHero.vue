  ...
  <section class="bg-white dark:bg-gray-900">
    ...
    <div v-else-if="status === 'error'">
      ...
      <div class="gap-16 items-center py-8 px-4 mx-auto max-w-screen-lg lg:grid lg:grid-cols-2 lg:py-16 lg:px-6">
        <div class="font-light text-gray-500 sm:text-lg dark:text-gray-400">
          ...
          <p class="mb-4">We are dedicated to transforming educational institutions through innovative consulting
            solutions and strategic guidance.</p>
        </div>
        <div class="grid grid-cols-2 gap-4 mt-8">
    ...
    <div v-else>
      <div v-show="data?.data?.status === 'published'"
        ...
        <div class="grid grid-cols-2 gap-4 mt-8">
          <NuxtImg class="w-full rounded-lg" :src="`${config.public.apiBaseUrl}/assets/${data.data.image_1}`"
            :quality="85" format="webp" loading="lazy" alt="About Page Image 1" preset="thumbnail" placeholder />
          <NuxtImg class="mt-4 w-full lg:mt-10 rounded-lg"
            :src="`${config.public.apiBaseUrl}/assets/${data.data.image_2}`" :quality="85" format="webp" loading="lazy"
            alt="About Page Image 2" preset="thumbnail" />
        </div>
      </div>
    </div>
  </section>
