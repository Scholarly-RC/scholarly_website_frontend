...
<template>
  <div v-show="props.team_member.status === 'published'" class="p-4">
    <div class="text-center text-gray-500 dark:text-gray-400 shadow-md rounded-lg cursor-pointer">
      <NuxtImg class="mx-auto mb-4 w-32 sm:w-36 h-32 sm:h-36 rounded-full"
        :src="`${config.public.apiBaseUrl}/assets/${team_member.image}`" :quality="85" format="webp" loading="lazy"
        alt="Team Member Image" preset="avatar" placeholder />
