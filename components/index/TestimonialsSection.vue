      ...
      <div v-if="status === 'pending'" class="overflow-hidden">
        <div class="mb-10 px-4">
          <div class="w-full max-w-lg mx-auto text-center">
            ...
            <div class="flex flex-col sm:flex-row items-center justify-center mt-6 space-y-3 sm:space-y-0 sm:space-x-3">
              ...
              <div class="flex flex-col sm:flex-row sm:items-center sm:divide-x-2">
                <div class="pr-0 sm:pr-3 w-24 h-4 bg-gray-400 dark:bg-gray-600 rounded animate-pulse"></div>
                <div class="pl-0 sm:pl-3 w-24 h-4 bg-gray-400 dark:bg-gray-600 rounded animate-pulse"></div>
              </div>
            </div>
          </div>
        </div>
          ...
          <swiper-slide v-for="testimonial in data?.data" :key="testimonial.id" class="mb-10 px-4">
            <figure v-show="testimonial.status === 'published'" class="w-full max-w-lg mx-auto text-center">
              <Icon name="heroicons:chat-bubble-left-right"
                class="h-10 md:h-12 mx-auto mb-3 text-gray-400 dark:text-gray-600" />
              <blockquote>
              ...
              <figcaption
                ...
                <NuxtImg class="w-10 h-10 rounded-full" :src="`${config.public.apiBaseUrl}/assets/${testimonial.image}`"
                  :quality="85" format="webp" loading="lazy" alt="Profile Picture" preset="avatar" placeholder />
                <div
                  class="flex flex-col sm:flex-row sm:items-center sm:divide-x-2 divide-gray-500 dark:divide-gray-700">
                  <div class="pr-0 sm:pr-3 font-medium text-gray-900 dark:text-white text-center sm:text-left">
                    {{ testimonial.name }}
                  </div>
                  <div
